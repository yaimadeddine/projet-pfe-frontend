...
export default function MaterielList() {
  ...
  useEffect(() => {
    axios.get("http://127.0.0.1:8000/api/fourniture/list")
    .then(res => {
  ...
  const deleteMateriel = async (e, id) => {
    ...
    const res = await axios.delete(
      `http://127.0.0.1:8000/api/delete/fourniture/${id}`
    );
  ...
  const tableRow = data.map((data, index) => {
    return(
      ...
      <td>{data.type_fourniture_name}</td>
      <td>{data.materiel_id}</td>
      <td>{data.materiel_matricule}</td>
      <td>{data.contrat_matricule}</td>
      <td className='col-sm-4 text-center'>
      <Link to={`/update/fourniture/${data.id}`} className="btn btn-info mr-2">
            Modifier
          </Link>
                ...
                >
                  <thead>
                    <tr>
                      ...
                      <th className='col-sm-1 text-center'>materiel</th>
